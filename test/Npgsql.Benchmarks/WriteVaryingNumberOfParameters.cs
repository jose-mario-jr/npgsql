...
public class WriteVaryingNumberOfParameters
{
    ...
    public void Setup()
    {
        ...
        using (var cmd = new NpgsqlCommandOrig($"CREATE FUNCTION pg_temp.swallow({funcParams}) RETURNS void AS 'BEGIN END;' LANGUAGE 'plpgsql'", _conn))
            cmd.ExecuteNonQuery();

        var cmdParams = string.Join(",", Enumerable.Range(0, NumParams).Select(i => $"@p{i}"));
        _cmd = new NpgsqlCommandOrig($"SELECT pg_temp.swallow({cmdParams})", _conn);
